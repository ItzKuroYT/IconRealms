...
async function handleApi(req, res) {
  try {
    ...
    if (route === "admin/staff-month") return adminStaffMonth(req, res);
...
function normalize(data) {
  ...
  if (data.site.staffOfMonth?.username) {
    const staffer = data.staff.find((person) => sameUser(person.username, data.site.staffOfMonth.username));
    data.site.staffOfMonth = {
      ...data.site.staffOfMonth,
      username: staffer?.username || data.site.staffOfMonth.username,
      rank: staffer?.rank || data.site.staffOfMonth.rank || "Staff",
      avatar: staffer?.avatar || data.site.staffOfMonth.avatar || avatar(data.site.staffOfMonth.username, 128)
    };
  }
...
async function createThread(req, res) {
  ...
  const { boardId, title, body, announcement, newsletter, images } = await readBody(req);
  ...
  const db = await readDb();
  const thread = makeThread({ boardId, title, body, images, author: user.username, announcement: Boolean(announcement && user.isAdmin), newsletter: Boolean(newsletter && user.isAdmin) });
...
async function adminStaffMonth(req, res) {
  const user = await currentUser(req);
  if (!user?.isAdmin) return send(res, 403, { error: "Admin required." });
  const body = await readBody(req);
  const db = await readDb();
  if (req.method === "DELETE" || body.action === "delete") {
    ...
    await writeDb(db);
    return send(res, 200, { ok: true, staff: db.staff, staffOfMonth: null });
  }
  const staffer = db.staff.find((item) => sameUser(item.username, body.username));
  if (!staffer) return send(res, 404, { error: "Staff member not found. Add them to the staff page first." });
  db.site.staffOfMonth = {
    username: staffer.username,
    rank: staffer.rank || "Staff",
    avatar: staffer.avatar || avatar(staffer.username, 128),
...
async function adminForums(req, res) {
  const user = await currentUser(req);
  if (!user?.isAdmin) return send(res, 403, { error: "Admin required." });
  const { threadId, action, boardId, title, body, announcement, newsletter, images } = await readBody(req);
  const db = await readDb();
  if (req.method === "POST") {
    if (!boardId || !title || !body) return send(res, 400, { error: "Board, title, and body are required." });
    const targetBoardId = newsletter
      ? firstFeedBoard("newsPage", "newsletter")
      : announcement
        ? firstFeedBoard("homeLatest", "announcements")
        : boardId;
  ...
  if (action === "newsletter") thread.newsletter = true;
  await writeDb(db);
  send(res, 200, { ok: true });
}
...
async function userNotifications(req, res) {
  ...
  if (action === "mark-read" || action === "clear") {
    ...
    if (!account) return send(res, 404, { error: "User not found." });
    const now = new Date().toISOString();
    account.lastNotificationSeenAt = now;
    if (action === "clear") account.notificationClearedAt = now;
    await writeDb(db);
    return send(res, 200, { ok: true, notificationClearedAt: account.notificationClearedAt || now });
  }
...
function makeThread({ boardId, title, body, images, author, announcement, newsletter }) {
  return { id: crypto.randomUUID(), boardId, title: filterText(String(title || "").slice(0, 140)), body: filterText(String(body || "").slice(0, 5000)), images: validateImages(images), author, announcement: Boolean(announcement), newsletter: Boolean(newsletter), locked: false, createdAt: new Date().toISOString(), replies: [] };
}
...
function normalizeThread(thread) {
  ...
  thread.announcement = Boolean(thread.announcement);
  thread.newsletter = Boolean(thread.newsletter);
  thread.replies = thread.replies.map((reply) => ({ ...reply, images: reply.images || [], createdAt: reply.createdAt || new Date().toISOString() }));
  return thread;
}

function firstFeedBoard(feedName, fallback) {
  const boards = config.contentFeeds?.[feedName]?.boardIds || [];
